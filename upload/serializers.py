...
class ProcessamentoFinalSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        for condo in condominios_data:
            condo_obj = existing_condos.get(condo['cnpj'])
            if not condo_obj:
                continue
            
            for func in condo.get('funcionarios', []):
                func_obj = existing_funcs.get(func['cpf'])
                if not func_obj:
                    continue
                
                for idx, mov in enumerate(func.get('movimentacoes', [])):
                    codigo_produto = (mov.get('codigo_produto') or '').strip()
                    if not codigo_produto:
                        nome_produto = (mov.get('produto') or '').strip()
                        codigo_produto = nome_produto[:50] if nome_produto else 'SEM_PRODUTO'
                    
                    prod_obj = existing_prods.get(codigo_produto)
                    if not prod_obj:
                        try:
                            prod_obj = Produto.objects.get(codigo_produto=codigo_produto)
                        except Produto.DoesNotExist:
                            prod_obj = Produto.objects.create(
                                codigo_produto=codigo_produto,
                                nome=mov.get('produto', codigo_produto)
                            )
                    ...
                    movimentacoes_to_create.append(MovimentacaoBeneficio(
                        ...
                        valor_beneficio=Decimal(str(mov.get('valor', 0))),
                        quantidade_dias=1
                    ))
                    registros_count += 1
